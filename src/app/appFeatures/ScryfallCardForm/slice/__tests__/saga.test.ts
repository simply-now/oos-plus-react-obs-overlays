...
import { scryfallCardFormSaga, getRepos } from '../saga';
...
describe('getRepos Saga', () => {
  ...
  it('should return error if username is empty', () => {
    ...
    expect(putDescriptor).toEqual(
      put(slice.scryfallCardFormActions.repoError(RepoErrorType.USERNAME_EMPTY)),
    );
  ...
  it('should dispatch the reposLoaded action if it requests the data successfully', () => {
    ...
    expect(putDescriptor).toEqual(
      put(slice.scryfallCardFormActions.reposLoaded(repos)),
    );
  });
  ...
  it('should dispatch the user not found error', () => {
    ...
    expect(putDescriptor).toEqual(
      put(slice.scryfallCardFormActions.repoError(RepoErrorType.USER_NOT_FOUND)),
    );
  });
  it('should dispatch the user has no repo error', () => {
    ...
    expect(putDescriptor).toEqual(
      put(
        slice.scryfallCardFormActions.repoError(RepoErrorType.USER_HAS_NO_REPO),
      ),
    );
  });
  it('should dispatch the github rate limit error', () => {
    ...
    expect(putDescriptor).toEqual(
      put(
        slice.scryfallCardFormActions.repoError(RepoErrorType.GITHUB_RATE_LIMIT),
      ),
    );
  });
  ...
  it('should dispatch the response error', () => {
    ...
    expect(putDescriptor).toEqual(
      put(slice.scryfallCardFormActions.repoError(RepoErrorType.RESPONSE_ERROR)),
    );
  });
});

describe('scryfallCardFormSaga Saga', () => {
  const scryfallCardFormIterator = scryfallCardFormSaga();
  it('should start task to watch for loadRepos action', () => {
    const takeLatestDescriptor = scryfallCardFormIterator.next().value;
    expect(takeLatestDescriptor).toEqual(
      takeLatest(slice.scryfallCardFormActions.loadRepos.type, getRepos),
    );
  });
});
